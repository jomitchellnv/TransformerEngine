...
def test_data_loading(load_test_data):
    ...
    assert cp1_logits.dim() == 3, f"CP=1 logits should be 3D (Batch, Seq, Vocab), got {cp1_logits.dim()}D"
    assert cp2_rank0_logits.dim() == 3, f"CP=2 rank 0 logits should be 3D (Batch, Seq, Vocab), got {cp2_rank0_logits.dim()}D"
    assert cp2_rank1_logits.dim() == 3, f"CP=2 rank 1 logits should be 3D (Batch, Seq, Vocab), got {cp2_rank1_logits.dim()}D"
